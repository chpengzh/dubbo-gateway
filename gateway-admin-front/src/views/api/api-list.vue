<template>
  <div>
    <el-row class="wrapper">
      <el-row>
        <el-col :span="12">
          <ApiIndexTabs tab-activate="all-service"/>
        </el-col>
      ...
      <el-table :data="tableData" fit highlight-current-row class="row-element" empty-text="暂无服务">
        ...
        <el-table-column key="dubboService" label="dubbo服务" :show-overflow-tooltip="true" min-width="300px">
          <template slot-scope="scope">{{scope.row.dubboService + '#' + scope.row.dubboMethod}}</template>
        </el-table-column>
        <el-table-column key="status" label="状态" :show-overflow-tooltip="true" width="100px">
          <template slot-scope="scope">{{scope.row.releaseVersion ? '已上线' : '未上线'}}</template>
        </el-table-column>
        <el-table-column key="modifyUser" label="变更人" :show-overflow-tooltip="true" width="100px">
        ...
        <el-table-column key="control-panel" label="" width="200px">
          <div slot-scope="scope">
            <el-link type="primary" :href="toApiEdit(scope.row)">编辑发布</el-link>
